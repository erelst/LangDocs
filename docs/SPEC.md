...
**Angka yang dipakai, diukur dari bank yang ada sekarang (564 kalimat panjang).** Inilah dasarnya,
dan ini yang tidak boleh memburuk:

| Checkpoint | Sekarang | Batas | Kenapa angka itu |
|---|---|---|---|
| Pola akhir kalimat (3 aksara terakhir) yang paling sering | `ますか` 14,7% | **di bawah 20%** | satu bentuk tanya tidak boleh mendominasi seperlima bank |
...
| Penanda relasi teratas (`ので`) | 39,5% | **di bawah 45%** | K3 mengizinkan banyak relasi, jadi menumpuk di satu penanda itu pilihan, bukan keharusan |
| Kalimat panjang yang klausanya disambung **sebab** | 67,4% | **di bawah 72%** | bentuk "karena X, maka Y" adalah bentuk termudah ditulis dan paling sulit dirasakan pengulangannya |
...
|---|---|
| Relasi dihitung sebagai **pasangan** (`ので+し`) | `ので` muncul bersama banyak pasangan berbeda, jadi tidak ada satu pasangan yang terlihat besar, padahal `ので` sendiri memegang **223 dari 564** kalimat |
...
Sesudah dibetulkan: `ので` **223** kalimat (39,5%), `から` **153** (27,1%), `し` **73** (12,9%).
...
|---|---|---|---|
| kerja & belajar | 25,5% | 22,7% | **+2,8** |
| publik & jasa | 15,7% | 10,8% | **+4,9** |
| orang asing | 6,6% | 2,4% | **+4,2** |
| teman & tetangga | 17,3% | 17,2% | **+0,1** |
| guru-murid | 1,0% | 3,1% | -2,1 |
| keluarga dekat | 33,9% | 36,8% | **-2,9** |

Kolom Deck dihitung dari 623 kalimat tertulis. Angka di tabel ini pernah basi (dihitung dengan
...
menyimpang, dan setelah lima batch medan makna tidak ada kelompok yang melewatinya lagi (tertinggi publik & jasa 4,9).

**Keluarga dekat kurang 2,9 poin, turun dari 6,3 setelah lima batch medan makna, dan itu
