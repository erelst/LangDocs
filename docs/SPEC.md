...
**Angka yang dipakai, diukur dari bank yang ada sekarang (573 kalimat panjang).** Inilah dasarnya,
...
| Pembuka kalimat yang paling sering | `この` 5,2% | **tidak ada satu pun di atas 8%** | pembuka adalah tempat pengulangan paling mudah terlihat pembaca; angkanya diukur, bukan dihitung tangan dari tiga pembuka teratas |
...
| Kalimat panjang yang klausanya disambung **sebab** | 67,5% | **di bawah 72%** | bentuk "karena X, maka Y" adalah bentuk termudah ditulis dan paling sulit dirasakan pengulangannya |
...
|---|---|
| Relasi dihitung sebagai **pasangan** (`ので+し`) | `ので` muncul bersama banyak pasangan berbeda, jadi tidak ada satu pasangan yang terlihat besar, padahal `ので` sendiri memegang **224 dari 573** kalimat |
...
Sesudah dibetulkan: `ので` **224** kalimat (39,1%), `から` **159** (27,7%), `し` **73** (12,7%).
...
|---|---|---|---|
| kerja & belajar | 25,2% | 22,7% | **+2,5** |
| publik & jasa | 15,5% | 10,8% | **+4,7** |
| orang asing | 6,5% | 2,4% | **+4,1** |
| teman & tetangga | 18,5% | 17,2% | **+1,3** |
| guru-murid | 0,9% | 3,1% | -2,2 |
| keluarga dekat | 33,4% | 36,8% | **-3,4** |

Kolom Deck dihitung dari 632 kalimat tertulis. Angka di tabel ini pernah basi (dihitung dengan
...
menyimpang, dan setelah enam batch medan makna tidak ada kelompok yang melewatinya lagi (tertinggi publik & jasa 4,7).

**Keluarga dekat kurang 3,4 poin, turun dari 6,3 setelah enam batch medan makna, dan itu
