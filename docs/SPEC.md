...
**Angka yang dipakai, diukur dari bank yang ada sekarang (557 kalimat panjang).** Inilah dasarnya,
dan ini yang tidak boleh memburuk:

| Checkpoint | Sekarang | Batas | Kenapa angka itu |
|---|---|---|---|
| Pola akhir kalimat (3 aksara terakhir) yang paling sering | `ますか` 14,9% | **di bawah 20%** | satu bentuk tanya tidak boleh mendominasi seperlima bank |
...
| Penanda relasi teratas (`ので`) | 40,0% | **di bawah 45%** | K3 mengizinkan banyak relasi, jadi menumpuk di satu penanda itu pilihan, bukan keharusan |
| Kalimat panjang yang klausanya disambung **sebab** | 67,0% | **di bawah 72%** | bentuk "karena X, maka Y" adalah bentuk termudah ditulis dan paling sulit dirasakan pengulangannya |
...
|---|---|
| Relasi dihitung sebagai **pasangan** (`ので+し`) | `ので` muncul bersama banyak pasangan berbeda, jadi tidak ada satu pasangan yang terlihat besar, padahal `ので` sendiri memegang **223 dari 557** kalimat |
...
Sesudah dibetulkan: `ので` **223** kalimat (40,0%), `から` **146** (26,2%), `し` **73** (13,1%).
...
|---|---|---|---|
| kerja & belajar | 25,8% | 22,7% | **+3,1** |
| publik & jasa | 15,9% | 10,8% | **+5,1** |
| orang asing | 6,7% | 2,4% | **+4,3** |
| teman & tetangga | 17,5% | 17,2% | **+0,3** |
| guru-murid | 1,0% | 3,1% | -2,1 |
| keluarga dekat | 33,1% | 36,8% | **-3,7** |

Kolom Deck dihitung dari 616 kalimat tertulis. Angka di tabel ini pernah basi (dihitung dengan
...
menyimpang, dan setelah empat batch medan makna itu berlaku untuk publik & jasa (+5,1), yang sempat turun ke 4,9 lalu naik lagi.

**Keluarga dekat kurang 3,7 poin, turun dari 6,3 setelah empat batch medan makna, dan itu
