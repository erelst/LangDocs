...
window.BANK = (window.BANK || []).concat([
  {
    ...
    blocks: [
      { sp: 'B', t: ['すみません', '、', '途中', 'まで', 'は', '分かった', 'の', 'です', 'が', '、', 'そこ', 'から', '先', 'が', '追え', 'ません', 'でした', '。'] },
      { sp: 'A', t: ['どの', 'あたり', 'です', 'か', '。', 'もう', '一度', '言い', 'ます', 'よ', '。'] },
      { sp: 'B', t: ['三つ目', 'の', '手順', 'の', 'ところ', 'です', '。', 'そこ', 'だけ', 'もう', '一度', 'お願い', 'できます', 'か', '。'] },
      { sp: 'A', t: ['もちろん', 'です', '。', '番号', 'を', '振り', 'ながら', '言い', 'ます', 'ね', '。'] },
      { sp: 'B', t: ['ありがとう', 'ございます', '。', '今度', 'は', '、', '自分', 'の', '言葉', 'で', '繰り返して', 'みます', '。'] },
      { sp: 'A', t: ['そう', 'して', 'もらえる', 'と', '、', 'こちら', 'も', '間違い', 'に', '気が付き', 'ます', '。'] },
      { sp: 'B', t: ['では', '、', '一度', '言って', 'みます', '。', '違って', 'いたら', '止めて', 'ください', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['先日', '、', '説明', 'を', '聞いて', 'いる', '間', 'に', '、', '三', '回', 'ほど', '口', 'を', '出し', 'そう', 'に', 'なりました', '。'] },
      { t: ['どれ', 'も', '、', '途中', 'まで', '聞け', 'ば', '分かる', 'こと', 'だ', 'と', '思った', 'から', 'です', '。'] },
      { t: ['ただ', '、', '途中', 'で', '止める', 'と', '、', '話', 'の', '順番', 'が', '崩れる', 'の', 'が', '分かって', 'いた', 'ので', '、', '我慢', 'しました', '。'] },
      { t: ['最後', 'まで', '聞い', 'たら', '、', '私', 'が', '言い', 'たかった', 'こと', 'は', '、', 'すでに', '説明', 'に', '入って', 'いました', '。'] },
      { t: ['おかげ', 'で', '、', '余計', 'な', '質問', 'を', 'せ', 'ず', 'に', '済み', 'ました', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['まず', '、', 'なぜ', 'その', 'お願い', 'が', '出た', 'の', 'か', 'を', '確かめ', 'ました', '。'] },
      { t: ['事情', 'が', '分かった', 'ので', '、', '「', 'それは', '必要', 'です', 'ね', '」', 'と', '一度', '受け止め', 'ました', '。'] },
      { t: ['その', 'うえ', 'で', '、', '私', 'の', '手', 'が', '空いて', 'い', 'ない', '時間', 'を', '具体的', 'に', '伝え', 'ました', '。'] },
      { t: ['断って', '終わり', 'に', 'する', 'の', 'は', '避け', 'たかった', 'ので', '、', '二つ', 'の', '代案', 'を', '出し', 'ました', '。'] },
      { t: ['結局', '、', '片方', 'で', '進む', 'こと', 'に', 'なり', '、', '関係', 'は', '変わらず', 'に', '済み', 'ました', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['頼まれる', 'と', '、', '考える', '前', 'に', '「', '大丈夫', 'です', '」', 'と', '言って', 'しまう', 'ん', 'だ', 'よ', '。'] },
      { t: ['あと', 'で', '予定', 'を', '見て', '、', 'あっ', 'と', 'なる', 'こと', 'が', '毎回', 'ある', '。'] },
      { t: ['嫌', 'な', 'わけ', 'じゃ', 'ない', 'から', '余計', 'に', '断れ', 'ない', 'ん', 'だけど', '、', '引き受けた', '分', 'だけ', '自分', 'が', '苦しく', 'なる', '。'] },
      { t: ['この', '前', 'も', '、', '三つ', '同時', 'に', '受けて', '、', '週末', 'が', '全部', '潰れた', '。'] },
      { t: ['うまい', '断り方', 'を', '教えて', 'ほしい', 'わけ', 'じゃ', 'なくて', '、', 'なぜ', '言え', 'ない', 'の', 'か', 'を', '聞いて', 'ほしい', 'だけ', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['昨日', 'の', 'こと', 'で', '、', '一つ', 'だけ', '伝えて', 'おき', 'たい', 'こと', 'が', 'あります', '。'] },
      { t: ['断られた', 'こと', '自体', 'は', '、', '仕方', 'ない', 'と', '思って', 'います', '。', '理由', 'も', '分かり', 'ました', '。'] },
      { t: ['ただ', '、', '「', 'そんな', 'の', '無理', 'です', '」', 'と', '言われ', 'た', 'とき', '、', '聞かなければ', 'よかった', 'と', '思いました', '。'] },
      { t: ['次', 'から', '、', '同じ', 'こと', 'で', '聞き', 'にくく', 'なる', 'の', 'が', '正直', 'つらい', 'です', '。'] },
      { t: ['同じ', '断る', 'なら', '、', '理由', 'を', '先', 'に', '言って', 'いただける', 'と', '、', 'こちら', 'も', '納得', 'できます', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['褒め', 'られた', 'とき', 'は、', '二つ', 'の', '間', 'の', 'どちら', 'か', 'に', 'なり', 'がち', 'です', '。'] },
      { t: ['一つ', 'は', '「', 'そんな', 'こと', 'あり', 'ません', '」', 'と', '全部', '打ち消す', 'こと', 'で、', 'もう', '一つ', 'は', '「', 'そう', 'です', '」', 'と', '受ける', 'こと', 'です', '。'] },
      { t: ['どちら', 'も', 'ぎこちなく', 'なります', '。', '打ち消す', 'と', '相手', 'の', '言葉', 'まで', '返して', 'しまう', 'から', 'です', '。'] },
      { t: ['一番', '収まり', 'が', 'いい', 'の', 'は、', '「', 'ありがとう', 'ございます', '。', '手伝って', 'もらった', 'おかげ', 'です', '」', 'と', '言う', 'こと', 'です', '。'] },
      { t: ['これ', 'なら', '、', '謙遜', 'に', 'も', '自慢', 'に', 'も', 'なら', 'ず', 'に', '、', '事実', 'だけ', 'が', '残り', 'ます', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['少し', 'だけ', '、', '共有', 'して', 'おき', 'たい', 'こと', 'が', 'あります', '。'] },
      { t: ['この', '二週間', '、', '急ぎ', 'の', '依頼', 'が', '夕方', 'に', 'まとまって', '来て', 'います', '。'] },
      { t: ['その', 'ため', '、', 'その', '日', 'に', '決めて', 'いた', '予定', 'を', '毎回', 'ずらして', 'います', '。'] },
      { t: ['誰', 'か', 'が', '悪い', 'と', 'いう', 'こと', 'で', 'は', 'あり', 'ません', '。', '流れ', 'の', '問題', 'だ', 'と', '思って', 'います', '。'] },
      { t: ['もし', '可能', 'なら', '、', '午前', 'に', '分かる', 'よう', 'に', 'して', 'いただける', 'と', '助かり', 'ます', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['長く', '話して', 'いる', 'とき', 'は', '、', 'いきなり', '切らない', 'で', '三つ', 'の', '順', 'に', 'しよう', 'と', '決めて', 'います', '。'] },
      { t: ['まず', '、', '話', 'の', '中身', 'を', '二つ', 'に', 'まとめて', '言い', 'ます', '。', '相手', 'は', '聞いて', 'もらえた', 'と', '感じ', 'ます', '。'] },
      { t: ['次', 'に', '、', '自分', 'が', '次に', 'する', 'こと', 'を', '一つ', 'だけ', '言い', 'ます', '。'] },
      { t: ['最後', 'に', '、', '時間', 'を', 'くれた', 'こと', 'に', 'お礼', 'を', '言い', 'ます', '。'] },
      { t: ['これ', 'で', '、', '終わり', 'たい', 'と', '伝え', 'て', 'も', '、', '切った', 'よう', 'に', 'は', '聞こえ', 'ません', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['悪く', 'ない', 'こと', 'で', '「', 'すみません', '」', 'と', '言う', 'の', 'は', '、', '少し', 'ずつ', 'やめた', '方', 'が', 'いい', 'よ', '。'] },
      { t: ['毎回', '謝って', 'いると', '、', '本当', 'に', '謝る', 'べき', 'とき', 'に', '、', 'その', '言葉', 'が', '軽く', '聞こえ', 'て', 'しまう', '。'] },
      { t: ['待たせた', 'とき', 'なら', '、', '「', 'ありがとう', 'ございます', '」', 'で', '済む', 'こと', 'が', '多い', '。'] },
      { t: ['私', 'も', '前', 'は', 'そう', 'で、', '一日', 'に', '十', '回', '以上', '言って', 'いた', 'けど', '、', '今', 'は', '半分', '以下', 'です', '。'] },
      { t: ['謝る', '代わり', 'に', 'お礼', 'を', '言う', 'だけ', 'で', '、', '印象', 'は', 'ずいぶん', '変わります', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['今', 'すぐ', 'の', 'お返事', 'は', '控え', 'たい', 'の', 'です', 'が', '、', 'よろしい', 'でしょうか', '。'] },
      { t: ['条件', 'の', 'うち', '、', '二つ', 'が', 'まだ', '飲み込めて', 'い', 'ません', '。'] },
      { t: ['明日', 'の', '朝', 'まで', 'に', '、', '三つ', 'に', '絞って', 'お返事', 'します', '。'] },
      { t: ['すぐ', 'に', '「', 'はい', '」', 'と', '言って', '、', 'あと', 'で', '覆す', '方', 'が', 'お互い', 'に', '困る', 'と', '思います', 'ので', '。'] },
      { t: ['もし', 'それ', 'まで', 'に', '待て', 'ない', 'よう', 'でしたら', '、', '今夜', 'の', 'うち', 'に', '連絡', 'します', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['会議', 'で', '、', '同僚', 'が', '説明', 'の', '途中', 'だった', 'の', 'に', '、', '私', 'が', '口', 'を', '挟み', 'ました', '。'] },
      { t: ['話', 'の', '流れ', 'から', '、', '次', 'に', '何', 'を', '言う', 'の', 'か', '分かった', 'つもり', 'だった', 'から', 'です', '。'] },
      { t: ['ところが', '、', '同僚', 'が', '続け', 'た', '内容', 'は', '、', '私', 'が', '想像', 'した', 'の', 'と', '逆', 'でした', '。'] },
      { t: ['その場', 'で', '気まずく', 'なり', '、', 'あと', 'で', '二人', 'きり', 'の', 'とき', 'に', '謝り', 'ました', '。'] },
      { t: ['それ', 'から', 'は', '、', '相手', 'の', '話', 'が', '終わる', 'まで', '待つ', 'と', '決めて', 'います', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['会議', 'の', '進め方', 'について', '、', '三つ', 'お願い', 'が', 'あります', '。'] },
      { t: ['第一', 'に', '、', '話す', 'の', 'は', '一度', 'に', '一人', 'に', 'して', 'ください', '。', '重なると', '、', '聞こえ', 'ない', 'まま', '進み', 'ます', '。'] },
      { t: ['第二', 'に', '、', '質問', 'は', '途中', 'で', 'せ', 'ず', 'に', '、', '区切り', 'まで', '待って', 'ください', '。', '話', 'の', '順番', 'が', '崩れる', 'から', 'です', '。'] },
      { t: ['第三', 'に', '、', 'まだ', '発言', 'して', 'い', 'ない', '人', 'を', '先', 'に', '当て', 'ます', '。', '同じ', '人', 'だけ', 'に', 'ならない', 'ため', 'です', '。'] },
      { t: ['この', '三つ', 'で', '、', '会議', 'が', '三十分', '短く', 'なった', '回', 'も', 'あります', '。'] },
    ]
  },
]);
