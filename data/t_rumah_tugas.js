...
window.BANK = (window.BANK || []).concat([
  {
    ...
    blocks: [
      { sp: 'A', t: ['今日', '、', '私', 'は', '七時', 'に', '出る', 'から', '、', '長く', 'かかる', '物', 'は', '無理', 'なん', 'だ', '。'] },
      { sp: 'B', t: ['じゃあ', '、', '洗濯', 'は', '私', 'が', 'やる', 'よ', '。', '乾燥', 'まで', '時間', 'が', 'かかる', 'から', '。'] },
      { sp: 'A', t: ['助かる', '。', '私', 'は', 'ごみ', 'を', '出して', '、', '皿', 'だけ', '洗って', 'おく', 'ね', '。'] },
      { sp: 'B', t: ['皿', 'は', '置い', 'て', 'おいて', '。', '夜', 'に', 'まとめて', 'やる', 'から', '。'] },
      { sp: 'A', t: ['じゃあ', '、', '買い物', 'の', 'メモ', 'だけ', '書いて', 'おく', 'よ', '。', '帰り', 'に', '寄れる', 'から', '。'] },
      { sp: 'B', t: ['お願い', '。', '牛乳', 'が', '無い', 'の', 'と', '、', '洗剤', 'が', '少ない', 'の', 'を', '書いて', 'おいて', '。'] },
      { sp: 'A', t: ['わかった', '。', '行って', 'き', 'ます', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['先週', 'の', '水曜日', 'は', '、', '朝', 'から', '台所', 'の', '下', 'が', '濡れて', 'いて', '、', '蛇口', 'の', '付け根', 'から', '水', 'が', '漏れて', 'いました', '。'] },
      { t: ['その', '日', 'に', '限って', '、', '洗濯機', 'も', '途中', 'で', '止まって', 'しまい', '、', '二つ', 'が', '同時', 'に', '来ました', '。'] },
      { t: ['水', 'の', '方', 'を', '先', 'に', '直さ', 'ない', 'と', '床', 'まで', '傷む', 'ので', '、', 'そちら', 'を', '優先', 'しました', '。'] },
      { t: ['修理', 'の', '人', 'に', '電話', 'したら', '、', '午後', 'の', '三時', 'に', '来て', 'くれて', '、', '洗濯機', 'も', '同じ', '日', 'に', '見て', 'もらえ', 'ました', '。'] },
      { t: ['夜', 'に', 'は', '全部', '片付いて', '、', '床', 'も', '乾いて', 'いた', 'ので', '、', '思った', 'より', '早く', '終わり', 'ました', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['六時', 'に', '起きて', '、', 'まず', '洗濯', 'を', '回した', 'ら', '、', '乾かす', '時間', 'が', '稼げ', 'ました', '。'] },
      { t: ['その', 'あと', 'で', '、', '外', 'に', '干して', 'から', '、', '朝', 'ごはん', 'を', '作る', 'こと', 'に', 'しました', '。'] },
      { t: ['ごみ', 'の', '日', 'だった', 'ので', '、', '出す', 'の', 'を', '忘れ', 'ない', 'ように', '先', 'に', 'まとめ', 'ました', '。'] },
      { t: ['食べ終わって', 'から', '、', '皿', 'を', '洗って', '、', '着替えて', '、', '鞄', 'を', '確認', 'しました', '。'] },
      { t: ['全部', 'が', '順番', 'どおり', 'に', '進んだ', 'ので', '、', 'いつも', 'より', '五分', '早く', '家', 'を', '出る', 'こと', 'が', 'でき', 'ました', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['皿', 'を', '洗って', '、', '次', 'の', '日', 'に', 'また', '同じ', '数', 'だけ', '増えて', 'いる', 'の', '、', '本当に', '不思議', 'なん', 'だ', 'よ', '。'] },
      { t: ['洗濯', 'も', 'そう', 'で', '、', '乾かして', '畳んで', 'も', '、', '次の', '朝', 'に', 'は', 'また', '山', 'が', 'できて', 'いる', '。'] },
      { t: ['床', 'を', '拭いた', 'そば', 'から', '、', '誰', 'か', 'が', '通った', '跡', 'が', '付く', 'し', '、', 'きり', 'が', 'ない', '。'] },
      { t: ['休み', 'の', '日', 'に', '全部', 'やった', 'のに', '、', '夕方', 'に', 'は', 'もう', '元', 'に', '戻って', 'いた', '。'] },
      { t: ['手伝って', 'ほしい', 'と', 'か', '、', 'やり方', 'を', '変え', 'たい', 'わけ', 'じゃ', 'なくて', '、', 'ただ', '減ら', 'ない', 'こと', 'を', '言いたい', 'だけ', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['夜分', 'に', 'すみません', '。', '少し', 'だけ', 'お話', 'し', 'て', 'も', 'いい', 'です', 'か', '。'] },
      { t: ['最近', '、', '十一時', 'を', '過ぎて', 'から', '、', '上', 'の', '部屋', 'から', '洗濯機', 'の', '音', 'が', '聞こえ', 'ます', '。'] },
      { t: ['私', 'の', '部屋', 'は', 'ちょうど', '下', 'な', 'ので', '、', '寝る', 'とき', 'に', 'よく', '響く', 'の', 'です', '。'] },
      { t: ['生活', 'の', '時間', 'は', 'それぞれ', 'だ', 'と', '思う', 'ので', '、', '責めて', 'いる', 'わけ', 'で', 'は', 'あり', 'ません', '。'] },
      { t: ['ただ', '、', '十一時', 'まで', 'に', 'して', 'いただける', 'と', '、', 'こちら', 'も', '安心', 'して', '眠れ', 'ます', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['まず', '、', '一番', '多い', 'の', 'は', '燃える', 'ごみ', 'で', '、', 'これは', '週', 'に', '二回', 'です', '。'] },
      { t: ['プラスチック', 'と', '缶', 'は', '別', 'の', '袋', 'で', '、', '月曜', 'だけ', '出せ', 'ます', '。'] },
      { t: ['袋', 'は', '中', 'が', '見える', '物', 'を', '使って', 'ください', '。', '中身', 'が', '分から', 'ない', 'と', '、', '持って', '行って', 'もらえ', 'ません', '。'] },
      { t: ['分け', '方', 'を', '間違える', 'と', '、', 'その', '袋', 'だけ', '残され', 'ます', '。', 'そう', 'なった', 'ら', '、', '中', 'を', '分け', '直して', 'ください', '。'] },
      { t: ['分から', 'ない', '物', 'が', 'あっ', 'たら', '、', '出す', '前', 'に', '私', 'に', '聞いて', 'ください', '。', '表', 'に', '貼って', 'ある', '紙', 'も', '役に立ち', 'ます', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['今月', 'の', '支払い', '、', '電気', 'と', '水道', 'は', 'もう', '済ませた', 'よ', '。'] },
      { t: ['ガス', 'だけ', 'まだ', 'で', '、', '明後日', 'まで', 'に', '払え', 'ば', 'いい', 'と', '書いて', 'あった', '。'] },
      { t: ['領収書', 'は', 'いつも', 'の', '引き出し', 'に', '入れて', 'ある', 'から', '、', '必要', 'な', 'とき', 'に', '見て', '。'] },
      { t: ['金額', 'は', '先月', 'より', '少し', '高かった', 'けど', '、', '使った', '量', 'は', 'ほぼ', '同じ', 'だった', '。'] },
      { t: ['来月', 'は', 'もう', '少し', '安く', 'なる', 'ように', '、', 'エアコン', 'の', '使い方', 'を', '見直そう', 'と', '思って', 'いる', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['日曜', 'は', '一日', '使える', 'から', '、', '先', 'に', '順番', 'を', '決めて', 'おこう', '。'] },
      { t: ['一番', '汚れて', 'いる', 'の', 'は', '台所', 'の', '換気扇', 'な', 'ので', '、', '体力', 'の', 'ある', '午前', 'に', 'やる', '。'] },
      { t: ['私', 'が', '風呂', '場', '、', 'そちら', 'が', '窓', 'に', 'しよう', '。', '同じ', '部屋', 'で', 'やる', 'と', 'ぶつかる', 'から', '。'] },
      { t: ['昼', 'は', 'どこ', 'か', 'に', '出て', '食べて', '、', '午後', 'に', '残り', 'を', '片付ける', 'の', 'が', '効率', 'が', 'いい', '。'] },
      { t: ['寝室', 'だけ', 'は', '手', 'を', '付け', 'ない', 'で', 'おこう', '。', '疲れた', 'とき', 'に', '座れる', '場所', 'が', '要る', 'から', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['小さな', '物', 'ほど', '、', '後回し', 'に', 'し', 'ない', '方', 'が', 'いい', 'よ', '。'] },
      { t: ['五分', 'で', '終わる', 'こと', 'は', '、', 'その場', 'で', 'やって', 'しまった', '方', 'が', '結局', '楽', 'な', 'ん', 'だ', '。'] },
      { t: ['後', 'に', '回す', 'と', '、', '小さい', '物', 'が', '十', '個', '集まって', '、', '一日', '仕事', 'に', 'なる', '。'] },
      { t: ['それ', 'に', '、', '残って', 'いる', '物', 'が', '目', 'に', '入る', 'だけ', 'で', '、', '休んで', 'いて', 'も', '落ち着か', 'ない', '。'] },
      { t: ['だから', '、', '一個', 'だけ', 'でも', '、', '見つけ', 'た', 'とき', 'に', '片付ける', '癖', 'を', '付ける', 'と', 'いい', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['ごめん', '、', '今', 'すぐ', '手', 'を', '貸して', 'ほしい', 'の', '。'] },
      { t: ['洗濯', 'の', 'ホース', 'が', '外れて', '、', '水', 'が', 'あふれ', 'そう', 'な', 'ん', 'だ', '。'] },
      { t: ['やって', 'ほしい', 'の', 'は', '、', 'この', '部分', 'を', '押さえて', 'いて', 'もらう', 'だけ', 'で', 'いい', '。'] },
      { t: ['もし', '今', '手', 'が', '離せ', 'ない', 'なら', '、', '五分', 'だけ', '待つ', 'から', '、', '終わった', 'ら', '来て', '。'] },
      { t: ['それ', 'まで', '、', '私', 'が', 'タオル', 'で', '受け', 'て', 'おく', 'ね', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['初めて', '一人', 'で', '住んだ', 'とき', 'は', '、', '何', 'を', 'どれ', 'だけ', '買え', 'ば', 'いい', 'の', 'か', '分かって', 'い', 'ません', 'でした', '。'] },
      { t: ['安い', 'から', 'と', '、', '米', 'を', '十', 'キロ', 'も', '買って', '、', '湿気', 'の', '多い', '場所', 'に', '置いて', 'しまいました', '。'] },
      { t: ['一', 'か月', 'ほど', 'で', '虫', 'が', '付いて', '、', 'ほとんど', '捨てる', 'こと', 'に', 'なりました', '。'] },
      { t: ['それ', 'から', 'は', '、', '少し', '高く', 'て', 'も', '、', '二', 'キロ', 'ずつ', '買って', '、', '冷蔵庫', 'に', '入れ', 'て', 'います', '。'] },
      { t: ['お金', 'の', '節約', 'より', '、', '使い切れる', '量', 'を', '選ぶ', '方', 'が', '大事', 'だ', 'と', '覚え', 'ました', '。'] },
    ]
  },
  {
    ...
    blocks: [
      { t: ['住人', 'の', '皆様', 'に', 'お知らせ', 'です', '。', '来週', 'の', '火曜', '、', '水道', 'の', '工事', 'が', 'あります', '。'] },
      { t: ['水', 'が', '止まる', 'の', 'は', '、', '朝', 'の', '九時', 'から', '午後', 'の', '一時', 'まで', 'の', '四時間', 'です', '。'] },
      { t: ['その', '間', 'は', '、', '台所', 'も', '風呂', 'も', '使え', 'ません', '。', '洗濯', '機', 'も', '動き', 'ません', '。'] },
      { t: ['前', 'の', '日', 'まで', 'に', '、', '飲み水', 'と', '料理', '用', 'の', '水', 'を', '用意', 'して', 'ください', '。'] },
      { t: ['工事', 'が', '早く', '終わった', '場合', 'は', '、', '復旧', 'した', '時点', 'で', '掲示板', 'に', '貼り', 'ます', '。'] },
    ]
  },
]);
